...
void Bureaucrat::signForm(Form &a) const{
	if (this->grade <= a.getGradeToSign()) {
		a.beSigned(*this);
		std::cout << this->name << " signs " << a.getName() << std::endl;
	}
	else {
		std::cerr << this->name << " cannot sign " << a.getName() << " because ";
...
void Bureaucrat::executeForm(const Form &form) {
	try{
		...
		std::cout << name << " can't execute " << form.getName() << "! Reason: " << e.what() << std::endl;
	}
}
